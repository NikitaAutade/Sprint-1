...
def RentHouse(request):
    ...
    if u.userType == "Owner":
        # Owner sees all their houses regardless of approval
        h = House.objects.filter(user=u)
    else:
        # Public or other users see only houses that are approved by admin and available
        h = House.objects.filter(status='Available', is_approved=True)
    
    if request.method == "POST":
        search = request.POST.get('search')
        if u.userType == "Public":
            h = House.objects.filter(status='Available', city__icontains=search, is_approved=True)
        elif u.userType == "Owner":
            h = House.objects.filter(user=u, city__icontains=search)
        else:
            h = House.objects.filter(status='Available', city__icontains=search, is_approved=True)
...
@login_required
def house_request(request):
    user_profile = UserProfile.objects.get(user=request.user)

    if request.user.is_staff:
        # Admin view: show all unapproved house requests
        unapproved_houses = House.objects.filter(is_approved=False)
        return render(request, 'house-request.html', {
